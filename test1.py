...
formats = {
    ...
    1300: [24, [ ['', ''] ] + [
            ['{:08X}: ' + '{:02X} '*i + '   '*(24-i) + '  ', '{}'*i] for i in range(1, 25)
            ]
            ],

    1600: [32, [ ['', ''] ] + [
            ['{:08X}: ' + '{:02X} '*i + '   '*(24-i) + '  ', '{}'*i] for i in range(1, 33)
            ]
            ],
    }
...
def hex_print(data, winW):
        ...
        while addr < len(data):
            ...
            line = ''.join([fmt_head.format(addr, *data[addr:addr+bts], *[ 0 ]*32),
                            fmt_tail.format(*list(map(one_byte_to_ascii, data[addr:addr+bts])), *[ 0 ]*32)])
            sep_index = 10+3*bts//2
            line = line[:sep_index] + '| ' + line[sep_index:]
            res += line + '\n'
            addr += bts
...
if __name__ == "__main__":
    print(hex_print([ ord(c) for c in 'al;weoiurya,kjsdhvfauuweyrdfgfdgdfhdjfgjfyhhhu' ] + [ 0, 5, 8 ], 440))
