...
<<<<<<< HEAD
        ...
        self.build_search_dialog()
        self.build_replace_dialog()
...
<<<<<<< HEAD
    def show_search_dialog(self):
        ...
        self._last_dialog = dialog.dialog_type
    ...
    def show_find_again(self):
        if self._last_dialog:
            print(self._last_dialog)
            if self._last_dialog == "search": dialog = self.search_dialog
            elif self._last_dialog == "replace": dialog = self.replace_dialog
            else: return
            self.find_again(dialog)
        else: return
...
##        dialog.whole_word_checkbox
    ...
    def build_search_dialog(self):
        self.search_dialog = tk.Toplevel(self.root)

        parent = self.search_dialog
        parent.title("Search Dialog")
        parent.resizable(False, False)
        parent.transient(self.root)
        parent.withdraw()

## Set variables
        parent.dialog_type = "search"
        parent.string_ascii = tk.StringVar()
        parent.string_hex = tk.StringVar()
...
<<<<<<< HEAD
        tk.Radiobutton(find_frame, text = "Text string", variable=self.mode, value=EditMenu.ASCII, command=lambda dialog=parent: self.on_mode_change(dialog)
                       ).grid(row=0, column=0, sticky='w', padx=0, pady=0)
        tk.Radiobutton(find_frame, text = "Hex string", variable=self.mode, value=EditMenu.HEX, command=lambda dialog=parent: self.on_mode_change(dialog)
                       ).grid(row=2, column=0, sticky='w', padx=0, pady=0)

        tk.Button(find_frame, text="Text->Hex",  width=8, height=1, command=lambda dialog=parent: self.text_to_hex(dialog)
                  ).grid(row=0, column=1, columnspan=2, sticky='we', padx=1, pady=1)
        tk.Button(find_frame, text="Hex->Text", width=8, height=1, command=lambda dialog=parent: self.hex_to_text(dialog)
                  ).grid(row=2, column=1, columnspan=2, sticky='we', padx=1, pady=1)
        
        parent.ascii_entry = tk.Entry(find_frame, textvariable=parent.string_ascii, width=50)
        parent.ascii_entry.grid(row=1, column=0, columnspan=3, sticky="we", padx=0, pady=0)
        
        parent.hex_entry = tk.Entry(find_frame, textvariable=parent.string_hex)
        parent.hex_entry.grid(row=3, column=0, columnspan=3, sticky="we", padx=0, pady=0)
=======
...
<<<<<<< HEAD
        parent.match_case_checkbox = tk.Checkbutton(opt_frame, text="Match case", variable=self.matchCase_check)
        parent.match_case_checkbox.grid(row=0, column=1, sticky="w")
        parent.whole_word_checkbox = tk.Checkbutton(opt_frame, text="Whole word", variable=self.wholeWord_check)
        parent.whole_word_checkbox.grid(row=1, column=1, sticky="w")
        parent.wrap_around_checkbox = tk.Checkbutton(opt_frame, text="Wrap around", variable=self.wrapAround_check)
        parent.wrap_around_checkbox.grid(row=2, column=1, sticky="w")
=======
...
<<<<<<< HEAD
        tk.Button(buttons_frame, text="Close", command=lambda dialog=parent: self.hide_dialog(dialog)
                  ).grid(row=0, column=2, sticky="we", padx=5, pady=5)
        tk.Button(buttons_frame, text="FindDown", command=lambda dialog=parent: self.next_match("down", dialog)
                  ).grid(row=1, column=2, sticky="we", padx=5, pady=5)
        tk.Button(buttons_frame, text="FindUp", command=lambda dialog=parent: self.next_match("up", dialog)
                  ).grid(row=2, column=2, sticky="we", padx=5, pady=5)

        self.search_dialog.protocol(
            "WM_DELETE_WINDOW",
            lambda dialog=self.search_dialog: self.hide_dialog(dialog)
        )

    def build_replace_dialog(self):
        self.replace_dialog = tk.Toplevel(self.root)

        parent = self.replace_dialog
        parent.title("Replace Dialog")
        parent.resizable(False, False)
        parent.transient(self.root)
        parent.withdraw()

## Set variables
        parent.dialog_type = "replace"
        parent.string_ascii = tk.StringVar()
        parent.string_hex = tk.StringVar()
...
## Frame Find+Replace
        ...
        parent.ascii_entry = tk.Entry(find_frame, textvariable=parent.string_ascii)
        parent.ascii_entry.grid(row=1, column=0, columnspan=3, sticky="we", padx=0, pady=0)
        ...
        parent.replace_entry = tk.Entry(find_frame, textvariable=parent.string_ascii, width=50)
        parent.replace_entry.grid(row=5, column=0, columnspan=3, sticky="we", padx=0, pady=0)
...
## Frame Buttons
        ...
        tk.Button(buttons_frame, text="Replace+FindDown", command=lambda dialog=parent: self.next_match("down", dialog)
                  ).grid(row=1, column=2, sticky="we", padx=5, pady=5)
        tk.Button(buttons_frame, text="Replace+FindUp", command=lambda dialog=parent: self.next_match("up", dialog)
                  ).grid(row=2, column=2, sticky="we", padx=5, pady=5)
##        TODO:
##        Replace + ALL
        tk.Button(buttons_frame, text="ReplaceAll", command=lambda dialog=parent: self.next_match("up", dialog)
                  ).grid(row=3, column=2, sticky="we", padx=5, pady=5)
        
        self.replace_dialog.protocol(
            "WM_DELETE_WINDOW",
            lambda dialog=self.replace_dialog: self.hide_dialog(dialog)
        )
...
<<<<<<< HEAD
    ...
    def hide_dialog(self, dialog: tk.Toplevel):
        dialog.grab_release()
        dialog.withdraw()
...
>>>>>>> 03ef8fa61481004c46d427c8d3b2e6fecacafc03
        ...
        mode = self.mode.get()
<<<<<<< HEAD
        query = dialog.string_ascii.get() if mode == EditMenu.ASCII else dialog.string_hex.get()
=======
...
<<<<<<< HEAD
            dialog.string_ascii.set(self._last_query)
        else:
            dialog.string_hex.set(self._last_query)
